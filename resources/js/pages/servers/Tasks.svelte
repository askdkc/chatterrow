<script lang="ts">
    ...
    import { Hash, ListTodo, CalendarDays, CheckCircle2, Circle, User, ArrowLeft } from 'lucide-svelte';
    ...
    import type { ServerResource, ChannelResource, TodoResource, UserResource } from '@/types';
...
<div class="flex h-screen w-full overflow-hidden bg-[#313338] text-[#dbdee1]">
    ...
    <main class="flex min-w-0 flex-1 flex-col overflow-y-auto">
        <header class="sticky top-0 z-10 flex h-12 shrink-0 items-center gap-3 border-b border-black/10 bg-[#313338] px-4 dark:border-black/20">
            <Link href={`/servers/${server.id}`} class="rounded p-1 transition hover:bg-white/10">
                <ArrowLeft class="h-4 w-4" />
            </Link>
            <ListTodo class="h-4 w-4 text-[#5865f2]" />
            <h1 class="text-[15px] font-bold">タスク一覧</h1>
            <span class="ml-auto text-xs text-[#80848e]">
                {todos.filter((t) => !t.completed_at).length} 未完了 / {todos.length} 件
            </span>
        </header>
        ...
        <div class="flex-1 space-y-6 p-6">
            <!-- Channels as tasks -->
            <section>
                <h2 class="mb-2 text-xs font-bold uppercase tracking-wide text-[#80848e]">チャンネル（タスク）</h2>
                <div class="space-y-2">
                    {#each channels as channel (channel.id)}
                        <div class="flex items-center gap-3 rounded-lg bg-[#2b2d31] p-3 transition hover:bg-[#383a40]">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                {#if channel.description}
                                    <p class="truncate text-xs text-[#80848e]">{channel.description}</p>
                                {/if}
                            </div>
                            <div class="flex shrink-0 items-center gap-4 text-xs text-[#80848e]">
                                <span class="flex items-center gap-1">
                                    <CalendarDays class="h-3.5 w-3.5" />
                                    {channel.starts_on ? formatDue(channel.starts_on) : '開始未定'} 〜
                                    {channel.ends_on ? formatDue(channel.ends_on) : '期限未定'}
                                </span>
                                <span class="flex items-center gap-1">
                                    <ListTodo class="h-3.5 w-3.5" />
                                    {channel.open_todos_count ?? 0}/{channel.todos_count ?? 0}
                                </span>
                            </div>
                        </div>
                    {/each}
            ...
            <section>
                <h2 class="mb-2 text-xs font-bold uppercase tracking-wide text-[#80848e]">全タスク</h2>
                {#if todos.length === 0}
                    <p class="rounded-lg bg-[#2b2d31] p-6 text-center text-sm text-[#80848e]">
                        タスクがありません。チャンネル内で todo を作成してください。
                    </p>
                {:else}
                    <div class="space-y-2">
                        {#each todos as todo (todo.id)}
                            ...
                            >
                                {#if todo.completed_at}
                                    <CheckCircle2 class="h-5 w-5 shrink-0 text-[#23a559]" />
                                {:else}
                                    <Circle class="h-5 w-5 shrink-0 text-[#80848e]" />
                                {/if}
                                <div class="min-w-0 flex-1">
                                    <p class="truncate text-sm font-medium" class:line-through={todo.completed_at !== null}>
                                        {todo.title}
                                    </p>
                                    <Link
