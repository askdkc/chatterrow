<script lang="ts">
    ...
    function itemLabel(item: NotificationResource): string {
        return t('Notification from :author in :server / #:channel', {
    ...
    async function deleteAll() {
        if (
            ...
            (typeof window !== 'undefined' &&
                !window.confirm(t('Delete all notifications?')))
        ) {
            return;
        }
    ...
    >
        <div class="min-w-0 flex-1">
            <h2 class="truncate text-sm font-bold">
                {t('Mention notifications')}
            </h2>
            <p class="text-xs text-[#80848e]">
                {t('Unread: :count', { count: String(unreadTotal) })}
            </p>
        </div>
        <button
            type="button"
        ...
        <button
            ...
            aria-label={t('Delete all notifications')}
        >
            {#if deletingAll}
            ...
            {#each items as item (item.id)}
                ...
                >
                    ...
                    <button
                        ...
                        aria-label={t('Delete notification: :notification', {
                            notification: itemLabel(item),
                        })}
                    >
